<header class="header" id="header">
    <div class="header__toggle">
        <i class='bx bx-menu' id="header-toggle"></i>
    </div>

    <div class="header__img">
        <img src="assets/img/perfil.jpg" alt="">
    </div>
</header>

<div class="l-navbar" id="nav-bar">
    <nav class="nav">
        <div>
            <a href="#" class="nav__logo" title="Home">
                <i class='bx bx-layer nav__logo-icon'></i>
                <span class="nav__logo-name">BeetaBie</span>
            </a>

            <div class="nav__list">
                <a href="../admin" class="nav__link active">
                    <i class='bx bx-grid-alt nav__icon'></i>
                    <span class="nav__name">Dashboard</span>
                </a>

                <!-- <a href="Courses.php" class="nav__link">
                        <i class='bx bx-user nav__icon'></i>
                        <span class="nav__name">Courses</span>
                    </a> -->

                <a href="user-data" class="nav__link">
                    <i class='bx bx-message-square-detail nav__icon'></i>
                    <span class="nav__name">User Data</span> 
                </a>
                <a href="chatgroups" class="nav__link">
                    <i class='bx bx-message-square-detail nav__icon'></i>
                    <span class="nav__name">Chat Groups</span> 
                </a>
                <a href="sliders" class="nav__link">
                    <i class='bx bx-message-square-detail nav__icon'></i>
                    <span class="nav__name">Sliders</span> 
                </a>

                <a href="translation" class="nav__link">
                    <i class='bx bx-message-square-detail nav__icon'></i>
                    <span class="nav__name">Translation</span> 
                </a>

                <a href="states" class="nav__link">
                    <i class='bx bx-message-square-detail nav__icon'></i>
                    <span class="nav__name">States</span> 
                </a>
                <a href="districts" class="nav__link">
                    <i class='bx bx-message-square-detail nav__icon'></i>
                    <span class="nav__name">Districts</span> 
                </a>
                <a href="blocks" class="nav__link">
                    <i class='bx bx-message-square-detail nav__icon'></i>
                    <span class="nav__name">Blocks</span> 
                </a>

                <!-- <a class="nav__link" >
                <i class='bx bx-chevron-down nav__icon'></i>
                    <span class="nav__name">Users</span>
                </a>


                <a href="farmers" class="nav__link sublink">
                <i class='bx bx-user nav__icon'></i>
                    <span class="nav__name">Farmers</span>
                </a>
                <a href="business" class="nav__link sublink">
                    <i class='bx bxs-business nav__icon'></i>
                    <span class="nav__name">Bussiness</span>
                </a>
                <a href="experts" class="nav__link sublink">
                    <i class='bx bx-book nav__icon'></i>
                    <span class="nav__name">Experts</span>
                </a>
                <a href="students" class="nav__link sublink">
                    <i class='bx bxs-graduation nav__icon' ></i>
                    <span class="nav__name">Students</span>
                </a> -->

                <!-- <a href="cupons.php" class="nav__link">
                        <i class='bx bx-envelope nav__icon'></i>
                        <span class="nav__name">Cupons</span>
                    </a>
                    <a href="sells.php" class="nav__link">
                        <i class='bx bx-envelope nav__icon'></i>
                        <span class="nav__name">Sells</span>
                    </a> -->


            </div>
        </div>

        <a href="logout" class="nav__link">
            <i class='bx bx-log-out nav__icon'></i>
            <span class="nav__name">Log Out</span>
        </a>
    </nav>
</div>